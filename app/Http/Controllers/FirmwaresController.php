...
class FirmwaresController extends Controller
{
    ...
    public function getAdminList(Request $request)
    {
        ...
        $user_group = $user->groups()->first() ? $this->auth->getLoggedUser()->groups()->first()->id : null;
        if($user_group === 1 || $user_group === 2 || $user_group ===3){
            $results = $this->firmwareRepository->all($request->except(['page']));
        } else {
            $results = $this->firmwareRepository->allUser($request->except(['page']), $user->id);
        }
    ...
    {
        $user_group = $this->auth->getLoggedUser()->groups()->first() ? $this->auth->getLoggedUser()->groups()->first()->id : null;
        $category = $this->globalrepository->findcatForFirmware([1, 2]);

        return View::make('laravel-authentication-acl::admin.firmware.new')->with(['user_data' => $user_group, 'view_category' => $category]);
    ...
    public function getUpdate(Request $request)
    {
        ...
        return View::make('laravel-authentication-acl::admin.firmware.edit')->with(['user_data' => $user_group, 'data' => $result, 'view_category' => $category]);
    }
    ...
    public function getFirmwareByDevice(Request $request)
    {
        $results = $this->firmwareRepository->allWhereDevice($request->except(['page']), $request);
        //$category = $this->viewCategoryRepository->find($request->view_category_id);
        $category='';
        return View::make('admin.pages.firmwareCategoryFirmwarDevice')->with(['results' => $results, 'request' => $request, 'category' => $category]);
    }
