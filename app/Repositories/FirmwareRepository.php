...
class FirmwareRepository
{
    ...
    public function allWhereDevice(array $search_filters = [], $request)
    {
        $q = new Firmware();
        $per_page = Config::get('acl_base.list_per_page');
        return $q->where([['device_id', $request->device_id], ['fcategory_id', $request->fcategory_id], ['status', 'PUBLISHED']])
            ->orderBy('created_at', 'desc')
            ->paginate($per_page);
    }
